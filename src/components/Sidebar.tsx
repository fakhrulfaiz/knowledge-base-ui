...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-60 h-screen bg-white text-neutral-800 flex flex-col shrink-0 border-r border-neutral-200 select-none">
      ...
      <div className="h-14 px-4 flex items-center justify-between border-b border-neutral-200">
        <div className="flex items-center gap-2.5">
          ...
          <div className="font-semibold text-sm text-neutral-900 tracking-tight">
            Nexus Knowledge
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-4 text-xs">
        ...
        <div className="space-y-0.5">
          ...
          >
            <div className="flex items-center gap-2.5">
              <SlidersHorizontal className={`w-4 h-4 ${activeView === 'admin' ? 'text-white' : 'text-neutral-500'}`} />
              <span>Admin &amp; Resources</span>
            </div>
            {isOwnerOrAdmin ? (
              <span
                className={`px-1.5 py-0.5 rounded text-[9px] font-mono uppercase font-bold tracking-tight ${
                  activeView === 'admin' ? 'bg-neutral-800 text-emerald-300' : 'bg-emerald-100 text-emerald-800'
                }`}
              >
                ADMIN
              </span>
              ...
              <span
                className={`px-1.5 py-0.5 rounded text-[9px] font-mono uppercase font-bold tracking-tight ${
                  activeView === 'admin' ? 'bg-neutral-800 text-blue-200' : 'bg-blue-100 text-blue-800'
                }`}
              >
                LEAD
              </span>
            ) : (
              <span className="px-1.5 py-0.5 rounded bg-neutral-100 text-neutral-400 text-[9px] font-mono flex items-center gap-0.5">
        ...
        <div className="space-y-1 pt-1">
          ...
          <div className="pt-1">
            <button
              ...
              className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-md text-xs font-medium text-neutral-600 hover:bg-neutral-50 cursor-pointer"
            >
              <div className="flex items-center gap-2">
            ...
            {sharedExpanded && (
              <div className="pl-4 mt-0.5 space-y-0.5 border-l border-neutral-200 ml-3">
                {/* Organization */}
                <button
                  onClick={() => {
                    onSelectView('collections');
        ...
        <div className="pt-2">
          <div className="px-2 text-[10px] font-semibold uppercase tracking-wider text-neutral-400 mb-1">
            Collections
          </div>
      ...
      <div className="p-3 border-t border-neutral-200 bg-neutral-50/70 space-y-2.5">
        {/* Personal Storage Quota Widget */}
        <div className="p-2 bg-white rounded-lg border border-neutral-200 space-y-1 shadow-2xs">
        ...
        <div
          ...
          className="flex items-center justify-between text-xs cursor-pointer hover:bg-neutral-100 p-1 rounded-md transition-colors"
          title="Click to open Admin & Resource Governance"
        >
          <div className="min-w-0">
            <div className="font-semibold text-neutral-900 truncate flex items-center gap-1.5">
        ...
        {onChangeUserRole && (
          <div className="pt-1 flex items-center justify-between gap-1 text-[10px]">
            <span className="text-neutral-400">Access Role:</span>
            <select
              value={currentUser.systemRole}
              onChange={(e) => onChangeUserRole(e.target.value as SystemRole)}
              className="bg-white border border-neutral-300 rounded px-1.5 py-0.5 text-[10px] font-mono font-medium text-neutral-800 cursor-pointer"
            >
